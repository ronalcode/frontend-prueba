...
const getProvider = (id: number) => {
  restApi.get(`/proveedores/${id}`).then((response) => {
    provider.value.nombre = response.data.nombre;
    provider.value.direccion = response.data.direccion;
    provider.value.telefono = response.data.telefono;
    provider.value.email = response.data.email;
  });
};

onMounted(() => {
  const route = useRoute();
  provider.value.id = Number(route.params.id);
  getProvider(provider.value.id);
});
...
const saveProvider = () => {
  if (provider.value.nombre.trim() === '') {
    ...
    sendRequest('PUT', params, '/proveedores', 'Proveedor modificado correctamente');
  }
};
</script>
...
<template>
  <div class="row mt-3">
    <div class="col-md-6 offset-md-3">
      <div class="card">
        <div class="card-header bg-dark text-white text-center">Editar proveedor</div>
        <div class="card-body">
